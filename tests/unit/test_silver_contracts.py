...
from agentic_energy.common.corrections import constraint_is_binding, latest_by_natural_key
...
def test_all_retained_silver_contracts_are_materialized_views() -> None:
    for filename, table in SILVER_FILES.items():
        assert table in _declared_dataset(PIPELINE / "silver" / filename)
        source = (PIPELINE / "silver" / filename).read_text()
...
def test_binding_rule_is_exact_non_zero_and_raw_rows_are_not_filtered() -> None:
    ...
    source = (PIPELINE / "silver" / "silver_constraints.py").read_text()
