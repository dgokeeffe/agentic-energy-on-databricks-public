...
from agentic_energy.common.quality import registration_enrichment_quality
...
def test_open_ended_registration_remains_effective_and_spark_contract_matches() -> None:
    ...
    pipeline = (Path(__file__).resolve().parents[2] / "src/agentic_energy/silver/silver_facilities.py").read_text()
    bronze = (Path(__file__).resolve().parents[2] / "src/agentic_energy/bronze/bronze_registration.py").read_text()
