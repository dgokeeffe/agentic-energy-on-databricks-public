# Legacy local-fixture architecture (superseded for NEMWEB)

> This page documents the retained JSONL compatibility runner only. The primary
> workshop deployment is the dedicated NEMWEB Lakeflow architecture in
> `docs/nemweb-operations.md` and `docs/nemweb-migration-manifest.md`.

## Legacy decision

Maintain one ingestion workflow through Silver. Do not create one pipeline or
job per source. Source-specific behavior is data in the metadata contract and
parser/adapter registries; orchestration is generic.
...
Lakebase: source_metadata, parser versions, schedules, quality rules
                         │
                         ▼
             one serverless dispatcher job
                         │
        validates + snapshots metadata for run_id
                         │
              ┌──────────┴──────────┐
              ▼                     ▼
      generic acquire task    generic refine task
       (serverless Python)    (serverless/Lakeflow)
              │                     │
              └──────► Bronze ─► Quarantine ─► Silver
                                             │
                                             ▼
                                      Gold projections
                                             │
...
- a pre-existing UC catalog/schema with permission to create objects; the bundle
  creates and owns the managed landing Volume, so do not pre-create it;
- the account-level principals and UC permissions needed for the bundle-managed
  Volume grants;
