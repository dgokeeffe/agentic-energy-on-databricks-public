...
Facilitator-only technical reference for the layer contracts, per-record
lineage, and the datasets each layer produces. All record-level examples in
this document are the
**actual output** of the deterministic fixture profile:

```bash
uv run --project nemweb_foundation agentic-energy-local-fixture --output output
...
(`pipeline.py`, `_ingestion_timestamp`), replaying the run
...
Emitted per row into `reason_codes` (`pipeline.py`):
...
The acceptance equations from [`workshop-acceptance.md`](facilitator/workshop-acceptance.md),
...
Deployed (the `agentic_energy_job` bundle resource) — same tree, per run, on a
...
Supply identity-qualified dev values as `<catalog>` / `<schema>` /
`<landing_volume>`; do not commit workspace-specific identifiers. The immutable
per-run manifest under the Volume — not
...
The bundle's `control_plane.sql` defines the Lakebase (Postgres) control
...
uv run --project nemweb_foundation agentic-energy-local-fixture --output output-replay
...
The foundation run documented above is recorded as a dated evidence record and
linked from the participant miniwiki. The miniwiki carries intent and handoff;
commands, counts, `metadata_sha256`, acceptance equations, quarantine reason
codes, and DST offsets remain in the evidence record.

Verified state at time of writing:

| Check | Result |
|---|---|
| Evidence page | Present and linked from the miniwiki |
| Contract suite | 24 passed |
| Replay hashes | Identical for the deterministic fixture run |
| Human review | Recorded separately from automated test evidence |

The verbatim console capture and replay hashes are committed as a dated record:
...
- **The handoff is not asserted by the test suite, by design.** Human decisions
  and session notes remain review artifacts; deterministic behavior belongs in
  implementation tests and dated evidence pages.
- **The miniwiki is committed and shared through Git.** A page is continuity,
  not an automatic gate; keep human approval separate from agent-authored status.

The edge cases this document already bears on include malformed rows and
market timestamps. Both behaved **correctly** for the checked-in fixtures: all
three bad rows were isolated with reason codes, and the AEDT/AEST offsets resolved
properly. Additional defects may trigger on input shapes
...
- [`workshop-acceptance.md`](facilitator/workshop-acceptance.md) — the acceptance gate these equations come from
...
- [`test-evidence/`](test-evidence/) — dated run records and reproducible fixture evidence
