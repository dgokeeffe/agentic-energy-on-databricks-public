...
from agentic_energy.nemweb.lander import (
    ArchiveInput,
    LanderLimits,
    NemwebClient,
    _discover_live,
...
from agentic_energy.nemweb.source_registry import get_subject_by_key, subjects_for_scope
...
def _snapshot_inputs(root: Path, mode: str, scope: str) -> list[ArchiveInput]:
    ...
    for item in manifest["artifacts"]:
        ...
        result.append(ArchiveInput(
            ...
            (root / item["path"]).read_bytes(), item["source_url"], mode,
...
def main() -> int:
    ...
    print(json.dumps({"run_id": args.cycle_id, "status": status, "inserted_source_rows": inserted, "rejections": len(batch.rejects)}, sort_keys=True))
