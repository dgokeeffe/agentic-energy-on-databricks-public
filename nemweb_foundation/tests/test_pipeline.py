...
import agentic_energy.cli as primary_cli
import agentic_energy.legacy_cli as legacy_cli
...
def test_legacy_local_fixture_contract_and_idempotency(tmp_path):
...
def test_legacy_live_metadata_path_uses_generic_pipeline(monkeypatch, tmp_path):
...
def test_legacy_fixture_mode_uses_declared_ingestion_timestamp(tmp_path):
    """The compatibility demo stays reproducible, with no wall clock in fixture mode."""
...
def test_legacy_live_mode_stamps_the_real_ingestion_instant(monkeypatch, tmp_path):
    """Legacy live-mode freshness remains real even though NEMWEB is now primary.
...
def test_validation_change_preserves_the_legacy_fixture_baseline(tmp_path):
    """The compatibility fixture keeps its established row-accounting contract."""
    root, metadata_path, metadata = _external_contract(tmp_path)
    metadata_path.write_text(json.dumps(metadata))
...
def test_primary_cli_routes_named_nemweb_commands_without_legacy_fallback(monkeypatch):
    calls = []

    def load(module_name):
        calls.append(module_name)
        return lambda args: calls.append(list(args or ())) or 0

    monkeypatch.setattr(primary_cli, "_command", load)

    assert primary_cli.main(["land", "--mode", "snapshot"]) == 0
    assert calls == ["agentic_energy.nemweb.lander", ["--mode", "snapshot"]]


def test_primary_cli_requires_an_explicit_nemweb_command():
    with pytest.raises(SystemExit):
        primary_cli.main([])


def test_legacy_cli_is_an_explicit_compatibility_path(monkeypatch, tmp_path, capsys):
    captured = {}

    def local_pipeline(metadata, output, **kwargs):
        captured.update(metadata=metadata, output=output, kwargs=kwargs)
        return {"bronze": 1, "silver": 1, "quarantine": 0, "gold": 1}

    monkeypatch.setattr(legacy_cli, "run_pipeline", local_pipeline)
    output = tmp_path / "legacy"

    assert legacy_cli.main(["--output", str(output), "--mode", "fixture"]) == 0
    assert captured["output"] == str(output)
    assert captured["kwargs"]["mode"] == "fixture"
    assert "Local fixture pipeline complete" in capsys.readouterr().out
