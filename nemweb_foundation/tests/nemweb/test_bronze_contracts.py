...
CRITICAL_TABLES = {
    ...
    "bronze_nem_dispatch_unit_solution_t1",
    "bronze_nem_genunits",
    "bronze_nem_dudetail",
    "bronze_nem_dualloc",
}
...
def test_all_critical_bronze_tables_and_quarantines_are_declared() -> None:
    sources = list(PIPELINE.glob("bronze_*.py"))
...
def test_scada_is_actual_generation_and_t1_is_not_misrepresented() -> None:
    scada = (PIPELINE / "bronze_scada.py").read_text()
    unit_solution = (PIPELINE / "bronze_unit_solution.py").read_text()
    assert "actual_generation_mw" in scada
    assert "dispatch_target_mw" not in scada
    assert "actual_generation_mw >= 0" not in scada
    assert '"source.cadence": "daily_t_plus_1"' in unit_solution
    assert "five-minute availability is manufactured" in unit_solution
...
def test_bronze_never_deduplicates_distinct_report_versions() -> None:
    sources = "\n".join(path.read_text() for path in PIPELINE.glob("bronze_*.py"))
    assert "dropDuplicates" not in sources
    assert "distinct()" not in sources
    assert "row_number" not in sources
    assert "source_run_no" in sources
    assert "report_version" in (PIPELINE / "io.py").read_text()
