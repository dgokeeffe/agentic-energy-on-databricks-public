...
function ReadyRegionalOperations({ state }: { state: ReadyState }) {
  ...
  const modeLabel = state.mode === 'mock' ? 'Prepared non-live fixture' : focusedRow.sourceMode === 'snapshot' ? 'Snapshot data via Lakebase' : 'NEMWEB data via Lakebase';
  ...
  return (
    <Sheet open={journalOpen} onOpenChange={setJournalOpen}>
      ...
      <main className="app-shell">
            ...
            {capture && weakest ? (
              <>
                ...
                <p className="hero-summary">
                  ...
                  {focusedRow.sourceMode === 'snapshot' ? 'Prepared snapshot values, never live market evidence.' : 'Observed NEMWEB values; check source freshness before use.'}
                </p>
              </>
            ) : (
              <>
        ...
        {(state.stale || state.fuelRows === null || state.unitRows === null) && (
          <ul className="notice-row" aria-label="Data quality notices">
            ...
            {state.fuelRows === null && (
              <li className="notice notice-muted" role="status">
                <strong>Generation read unavailable</strong> — generation data could not be loaded. Reload to try again.
              </li>
            )}
            {state.unitRows === null && (
              <li className="notice notice-muted" role="status">
                <strong>Per-unit read unavailable</strong> — the map and unit ranking could not be loaded. Reload to try again.
              </li>
            )}
          </ul>
        )}
        ...
        <Card id="investigate" className="decision-cta">
          <CardContent>
            <div>
              ...
              <h2>Investigate the observation</h2>
              <p>
