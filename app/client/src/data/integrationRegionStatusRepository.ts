...
const regionStatusRowSchema = z.object({
...
type RegionStatusLoader = () => Promise<unknown[]>;

export function normaliseIntegrationRows(rows: unknown[]): RegionStatus[] {
  return rows.map((candidate) => {
    const row = regionStatusRowSchema.parse(candidate);
    return {
...
export class IntegrationRegionStatusRepository implements RegionStatusRepository {
  private readonly loader: RegionStatusLoader;

  constructor(loader: RegionStatusLoader) {
    this.loader = loader;
  }
