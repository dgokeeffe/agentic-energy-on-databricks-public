import { servingQueries } from './db/serving';
  ...
  await createApp({
    ...
    async onPluginsReady(appkit) {
      ...
      appkit.server.extend((app) => {
        for (const [path, query] of Object.entries(servingQueries)) {
          app.get(path, async (_req, res) => {
            try {
              const result = await appkit.lakebase.query(query);
              res.json(result.rows);
            } catch (error) {
              logDatabaseError(`Serving read ${path}`, error);
              res.status(503).json({ error: 'This data panel is temporarily unavailable.' });
            }
          });
        }
