...
test('prepared value-capture journey keeps region focus and journal context aligned', async ({ page }) => {
  ...
  // The native investigation sheet preserves keyboard focus on close.
  const cta = page.getByRole('button', { name: 'Investigate observation' });
  await cta.focus();
  ...
  // page CTA's "Investigate observation".
